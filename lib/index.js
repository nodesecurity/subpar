...
internals.conditionSchema = Joi.object({
  data: Joi.object(),
...
class Subpar {
  ...
  handle(handler, condition) {
    ...
    Joi.assert(condition, internals.conditionSchema, 'Invalid handler condition');

    const keys = Utils.deepKeys(condition);
    for (const key of keys) {
      this._handler = this._handler.when(Joi.ref(key), { is: Hoek.reach(condition, key), then: Joi.default(this.counter) });
    }

    const extender = Joi.object({ message: condition });
  ...
  start() {
    ...
    return this.initialize().then(() => {
      ...
      // $lab:coverage:off$
      this.server.log(['info'], `Started function '${this.name}' in ${this.production ? 'production' : 'development'} mode on port ${this.server.info.port} with handler for: POST ${this.path}`);
      // $lab:coverage:on$
    });
  }
}
